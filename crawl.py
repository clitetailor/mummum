...
def sumo():
    j = 1
    for i in range(2, 8):
        r = requests.get('http://comhopsumo.com/product-category/cac-thu-trong-tuan/com-van-phong-thu-' + str(i), proxies=proxies)
#	print r.content
...
def comnhanh():
    j = 1
    for i in range(2, 8):
        r = requests.get('http://comnhanh.vn/thuc-don-ngay-thu-' + str(i), proxies=proxies)
        soup = BeautifulSoup(r.content, 'lxml')
